...
# Import grasp sampler
from grasp_sampler import AntipodalGraspSampler, GraspPose
...
class CuroboPickPlaceController:
    ...
    def __init__(self, name, gripper, robot_articulation, franka_prim_path):
        ...
        # 🎯 抓取姿态采样器
        self.grasp_sampler = None  # 延迟初始化，在第一次抓取时创建
        self.current_grasp_pose = None  # 当前选择的抓取姿态
        self.target_object_path = None  # 目标物体路径
    ...
    def _get_target_pose(self, picking_position, placing_position, offset):
        """根据当前事件获取目标位姿（6状态版本，支持多样化抓取姿态）"""
        # 🎯 使用采样的抓取姿态（如果可用），否则使用默认朝下姿态
        if self.current_grasp_pose is not None:
            ee_quat = self.current_grasp_pose.orientation  # 使用采样的姿态
        else:
    ...
    def sample_grasp_pose(self, object_prim_path: str, num_samples: int = 30) -> bool:
        """为目标物体采样抓取姿态
        
        Args:
            object_prim_path: 目标物体的 USD 路径
            num_samples: 采样点数量
        ...
        try:
            ...
            if self.grasp_sampler is None:
                print("🎯 初始化抓取姿态采样器...")
                self.grasp_sampler = AntipodalGraspSampler(
                    gripper_width=0.08,  # Franka gripper max width
                    standoff_distance=0.02,
                    min_grasp_width=0.01
                )
            
            # 加载物体 mesh
            print(f"📦 加载物体 mesh: {object_prim_path}")
            if not self.grasp_sampler.load_mesh_from_usd(object_prim_path):
                print("❌ 无法加载物体 mesh")
                return False
            
            # 采样抓取姿态
            print(f"🔍 采样 {num_samples} 个候选抓取姿态...")
            candidate_grasps = self.grasp_sampler.sample_antipodal_grasps(
            ...
            # 选择最佳抓取姿态
            self.current_grasp_pose = self.grasp_sampler.get_best_grasp()
            print(f"✅ 选择最佳抓取姿态: quality={self.current_grasp_pose.quality:.3f}")
            print(f"   位置: {self.current_grasp_pose.position}")
            print(f"   姿态: {self.current_grasp_pose.orientation}")
            
            self.target_object_path = object_prim_path
            return True
    ...
    def reset(self):
        ...
        self.current_grasp_pose = None  # 重置抓取姿态
        if self.is_attached:
            self._detach_object()
...
def step_once(render: bool = True) -> bool:
    ...
    if my_world.is_playing():
        ...
        # 🎯 首次执行时：采样抓取姿态
        if my_controller.get_current_event() == 0 and my_controller.cmd_plan is None and my_controller.current_grasp_pose is None:
            ...
            print("🎯 开始采样抓取姿态...")
            print("="*60)
            success = my_controller.sample_grasp_pose(
                object_prim_path="/World/Vegetable_7",
                num_samples=30
            )
            if not success:
                print("⚠️ 抓取姿态采样失败，使用默认朝下姿态")
            print("="*60 + "\n")
