    ...
    def _get_target_pose(self, picking_position, placing_position, offset):
        ...
        if self.current_event == 0:  # 接近抓取
            ...
            if self.saved_pick_position is not None:
                pos = self.saved_pick_position + np.array([0, 0, lift_height]) + offset
            else:
                pos = picking_position + np.array([0, 0, lift_height]) + offset
            return (pos, ee_quat)
        elif self.current_event == 4:  # 接近放置
            pos = placing_position + np.array([0, 0, approach_height]) + offset
            return (pos, ee_quat)
...
def step_once(render: bool = True) -> bool:
    ...
    if my_world.is_playing():
        ...
        if current_event in [0, 1, 3, 4, 5, 7]:
            ...
            target_pos = None
            if current_event == 0:
                target_pos = picking_position + np.array([0, 0, 0.10]) + eef_lateral_offset
                ...
                if my_controller.saved_pick_position is not None:
                    target_pos = my_controller.saved_pick_position + np.array([0, 0, 0.10])
                else:
                    target_pos = picking_position + np.array([0, 0, 0.20]) + eef_lateral_offset
            elif current_event == 4:
                target_pos = placing_position + np.array([0, 0, 0.10]) + eef_lateral_offset
